...
if(isset($_POST["action"]))
{
	if($_POST["action"] == 'fetch')
	{
		...
		foreach($result as $row)
		{
			...
			{
				$status = '<button type="button" name="status_button" class="btn btn-primary btn-sm status_button" data-id="'.$row["doctor_id"].'" data-status="'.$row["doctor_status"].'">Activo</button>';
			}
			else
			{
				$status = '<button type="button" name="status_button" class="btn btn-danger btn-sm status_button" data-id="'.$row["doctor_id"].'" data-status="'.$row["doctor_status"].'">Inactivo</button>';
			}
			$sub_array[] = $status;
			$sub_array[] = '
			<div align="center">
	...
	if($_POST["action"] == 'Add')
	{
		...
		{
			$error = '<div class="alert alert-danger">La dirección de mail ya existe</div>';
		}
		else
		{
			...
			if($_FILES['doctor_profile_image']['name'] != '')
			{
			    ...
			    {
			        $error = "<div class='alert alert-danger'>Solo se permiten extensiones jpg, png</div>";
			    }
			    else if (($_FILES["doctor_profile_image"]["size"] > 2000000))
			    {
			       $error = "<div class='alert alert-danger'>Tamaño del archivo excede 2MB</div>";
			    }
			    else
			    {
			...
			if($error == '')
			{
				...
				$success = '<div class="alert alert-success">Doctor agregado</div>';
			}
		}
	...
	if($_POST["action"] == 'Edit')
	{
		...
		else
		{
			...
			if($error == '')
			{
				...
				$success = '<div class="alert alert-success">Información del doctor actualizada!</div>';
			}			
		}
	...
	if($_POST["action"] == 'change_status')
	{
		...
		echo '<div class="alert alert-success">Estado cambiado a '.$_POST['next_status'].'</div>';
	}
	...
	if($_POST["action"] == 'delete')
	{
		...
		echo '<div class="alert alert-success">Información del doctor eliminada</div>';
	}
}
