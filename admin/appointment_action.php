...
if(isset($_POST["action"]))
{
	...
	if($_POST["action"] == 'fetch_single')
	{
		...
		foreach($appointment_data as $appointment_row)
		{
			...
			<table class="table">
				<tr>
					<th width="40%" class="text-right">Nro. de Cita</th>
			...
			foreach($doctor_schedule_data as $doctor_schedule_row)
			{
				...
				<tr>
					<th width="40%" class="text-right">Fecha de Cita</th>
					<td>'.$doctor_schedule_row["doctor_schedule_date"].'</td>
				</tr>
				<tr>
					<th width="40%" class="text-right">Día de Cita</th>
			...
			$html .= '
				<tr>
					<th width="40%" class="text-right">Hora de Cita</th>
					<td>'.$appointment_row["appointment_time"].'</td>
				</tr>
				<tr>
					<th width="40%" class="text-right">Motivo de la Cita</th>
				...
				if($_SESSION['type'] == 'Admin')
				{
					if($appointment_row['patient_come_into_hospital'] == 'Yes')
					{
						if($appointment_row["status"] == 'Completed')
						{
							$html .= '
								<tr>
									<th width="40%" class="text-right">El paciente llegó al hospital</th>
									<td>Si</td>
								</tr>
								<tr>
	...
	if($_POST['action'] == 'change_appointment_status')
	{
		if($_SESSION['type'] == 'Admin')
		{
			...
			echo '<div class="alert alert-success">Cambio del estado de la cita en progreso</div>';
		}
		...
		if($_SESSION['type'] == 'Doctor')
		{
			if(isset($_POST['doctor_comment']))
			{
				...
				echo '<div class="alert alert-success">Cita completada</div>';
			}
		}
	}
